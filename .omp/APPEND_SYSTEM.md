...
Prefer the project skills `capno-context`, `capno-android`, `capno-ios-parity`, and `capno-packaging` when the request matches their descriptions. Use the custom tool `capno_context_lookup` for entity/context lookup and `capno_package` for packaging command previews or builds.
