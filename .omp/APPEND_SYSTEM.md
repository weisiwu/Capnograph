...
For raw Feishu robot text, use `capno_packflow_bot_command` first. For known Packflow-backed Android 打包 + 产物发送场景, use `capno_packflow_agent`; use `capno_packflow_query` for Packflow history/detail/config queries; use `capno_packflow` only for direct local runner packaging.
