...
- `capno_packflow_agent`：推荐给 AI/机器人调用的 Packflow 后台入口。触发本机 Packflow agent API，等待构建完成，读取 Packflow 产物，并可通知飞书机器人 `CapnoGraph OMP Bot`；Android APK 大文件通过 Packflow 下载链接交付，不上传文件二进制到飞书。
...
- `includeArtifactDownloadLinks`: 默认 true；在飞书通知中加入 Packflow `/api/artifacts/<artifactId>/download` 下载链接。
- `packflowPublicBaseUrl`: 可选公网 Packflow 地址；不设置时下载链接默认使用 `http://localhost:3001`，只适合同机或已登录本机 Packflow 的飞书客户端。
...
- 如需自定义二次消息，可再调用 `capno_feishu_send`。当前 APK 约 101 MB，超过飞书文件上传上限；飞书里发送 Packflow 下载链接、大小和 SHA-256，而不是文件本体。

已验证链路：`capno_packflow_agent` 触发 `Android Debug APK` build `f74cdf65-b3c6-4ad0-9ed4-2903f8ebe144`，Packflow 耗时 `25.090s`，产物 `apps/android/app/build/outputs/apk/debug/app-debug.apk`，飞书返回 `StatusCode=0` / `success`。2026-06-24 复验 build `df3a3a14-61db-48ba-811e-c87383e3566a` 耗时 `26.362s`，产物 `59d86d7d-a5c5-44b6-8a35-92e9991e80d3`，飞书补发通知成功并包含 Packflow 下载链接。
