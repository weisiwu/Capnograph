...
summary: CapnoEasy 业务、技术架构、审核和自动代码文档的统一入口。
created: 2026-07-19
updated: 2026-07-19
owner: 项目知识库维护者
...
!!! warning "证据优先级"
    当前源码、配置、数据库迁移和测试是第一证据源。人工手册用于沉淀已审核知识；`GitNexus 自动文档`用于导航和理解代码，但不能替代源码核验。
...
<span class="wiki-card__eyebrow">产品 · 临床业务 · 交付</span>

**[业务领域与端到端流程](business/domain-and-workflows.md)**

术语、参与者、监测/记录/报告流程和数据边界。
</div>
<div class="wiki-card" markdown>
...
flowchart TB
    ...
    accDescr: 源码、配置、数据库迁移和测试分别进入人工手册和 GitNexus 代码图谱，最终由 MkDocs 汇总为可搜索的静态知识站。
...
<figcaption><strong>文字摘要：</strong>源码同时进入人工手册和 GitNexus 图谱，二者经复核后由 MkDocs 汇总；源码仍是第一证据。</figcaption>
</figure>
