...
summary: CapnoEasy 行业背景、应用业务、技术架构和代码模块参考的统一入口。
created: 2026-07-19
updated: 2026-07-19
owner: 项目知识库维护者
...
CapnoEasy 项目知识库把业务语义、Android/iOS 两套独立架构和代码图谱连成一条可追溯的知识链。第一次进入建议先看图解导览；准备理解业务或修改代码时，直接进入对应栏目。

[五分钟图解导览](guide/visual-overview.md){ .md-button .md-button--primary }
[查看数据对象与风险](business/data-and-risks.md){ .md-button }
</div>
...
<div class="wiki-card" markdown>
<span class="wiki-card__eyebrow">Kotlin · Compose · Room</span>

**[Android 架构](architecture/android-architecture.md)**

Activity/Compose 页壳、全局状态、BLE Kit、Room、PDF 与热敏打印。
</div>
<div class="wiki-card" markdown>
<span class="wiki-card__eyebrow">SwiftUI · CoreBluetooth · PDFKit</span>

**[iOS 架构](architecture/ios-architecture.md)**

EnvironmentObject 视图树、CoreBluetooth、内存波形历史与 PDF 输出。
</div>
</div>
...
flowchart TB
    ...
    Source["源码 / 配置 / 迁移 / 测试"] --> Curated["业务 / 架构人工手册"]
...
- `docs/architecture/`：Android 与 iOS 两套独立架构页，分别以各自源码为证据。
...
!!! danger "发布与合并前优先处理"
    当前基线仍有报警边界、记录结束时间、数据持久化和患者数据处理等待确认项。对象关系、影响与业务决定见[数据对象与业务风险](business/data-and-risks.md)。
