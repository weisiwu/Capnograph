...
| iTextPDF | `com.itextpdf:itextpdf:5.5.13.4` | `:app`, `:hotmeltprint` | `PDFKit` 生成纸质报告单样式 PDF，按 section 分页、通过 page event 绘制可配置水印，并把 footer/signature 放在最后内容之后，打印模块也声明。 | 验证中文字体、表单字段、section 分页、可配置水印、最后页签字栏、手绘波形图片和 SAF 导出；库版本较旧，升级需回归 PDF 格式。 |
