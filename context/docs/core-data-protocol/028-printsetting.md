...
- 备注：共享打印设置对象；PDF 报告基础信息读取科室、床位号、住院号、姓名和性别，并保留 PDF 模板/水印、异常波形上下文秒数和详情趋势图选项

## 补充职责

PDF/小票共享打印配置；PDF 报告基础信息区会读取 `pdfDepart`、`pdfBedNumber`、`pdfIDNumber`、`name`、`gender`，不再展示年龄。PDF 模板模式、水印开关/文字/透明度和异常波形上下文秒数保存在该全局对象中，供后续 PDF 模板渲染使用。
