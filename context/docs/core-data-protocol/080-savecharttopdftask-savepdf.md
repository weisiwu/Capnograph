...
- 备注：生成临时 PDF，添加固定报告单页眉、表单信息、连续 14 秒波形段、固定参考值、签字和可选水印
...
临时文件非空才覆盖输出；失败删除 tmp。报告单样式包含医院名、固定标题 `呼气末二氧化碳监测报告单`、住院号/床位号/姓名/性别/年龄/身高/体重、从记录开头连续切分的最多三段 14 秒 CO2 波形、每段测量时间和指标行、固定文案 `EtCO2 参考值：32-42mmHg`、签字栏。
