...
- 备注：用 Canvas 手绘 15 秒波形网格、主刻度标签、EtCO2 正常范围带和按真实采样时间定位的 CO2 曲线

## 补充职责

生成纸质报告单风格的 CO2 波形 bitmap，包含密集网格、按 15 秒窗口标注的横轴、横纵轴主刻度标签、纵轴单位、EtCO2 正常范围带和黑色波形线；新记录的曲线点按 `sampleTimeMillis - segment.startMillis` 定位到真实横坐标，旧记录按点序和 `POINTS_PER_SECOND` 兜底。
...
波形 bitmap 宽高、PDF 图高、绘图区边界、网格数量、主刻度间隔、文字大小和线宽均来自 `PdfReportTemplateConfig`。`mmHg` 报告纵轴固定显示到 90，并标出 0/30/60/90 mmHg 主刻度；非 `mmHg` 单位沿用当前 CO2 量程并按 0、1/3、2/3、最大值标主刻度。EtCO2 正常范围以 `32-42mmHg` 为基准，按当前 CO2 单位换算后在图内用浅色范围带标出；未知单位不显示范围带。波形横轴按 `ReportSegment.durationMillis` 标注，当前每段固定 15 秒，横轴主标签步长为 5 秒。有 `sampleTimeMillis` 时按真实采样时间映射 x 坐标，无时间戳的旧数据按点序和 `POINTS_PER_SECOND` 兜底。PDF 波形不再依赖 MPAndroidChart 默认样式。
