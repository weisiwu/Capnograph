...
- 备注：用 Canvas 手绘可变时长异常上下文网格、主刻度标签、EtCO2 正常范围带和按真实采样时间定位的 CO2 曲线

## 补充职责

生成纸质报告单风格的 CO2 波形 bitmap，包含密集网格、按 `ReportSegment.durationMillis` 动态伸缩的横轴、横纵轴主刻度标签、纵轴单位、EtCO2 正常范围带和黑色波形线；新记录的曲线点按 `sampleTimeMillis - segment.startMillis` 定位到真实横坐标，旧记录按点序和 `POINTS_PER_SECOND` 兜底。
