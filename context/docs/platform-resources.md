...
| PACKFLOW | Packflow backend configs | 构建平台 | `context/docs/platform-resources/PACKFLOW.md` | 本机 Packflow 后台 CapnoGraph Android debug/release 与 iOS release 构建配置；`capno_packflow_bot_command` / `capno_packflow_agent` / `capno_packflow_query` 接入 AI/飞书机器人，debug APK 大包通过 Packflow 公开 token 下载链接交付 |
