...
定位入口：`context/entity-id-mapping.md`（L252）。
领域：页面函数。

## 实体定位

- 实体：`BaseActivity.onBackPressed`
- ID / 别名：back intercept, 返回拦截
- 源文件：`app/src/main/java/com/wldmedical/capnoeasy/pages/BaseActivity.kt`
- 原始补充上下文：`.cursor/rules/project-memory.mdc`
- 备注：一级页最后 Activity 返回时展示退出确认

## 补充职责

一级页最后 Activity 返回时展示退出确认。
...
- 主要实体或方法：`BaseActivity.onBackPressed`
