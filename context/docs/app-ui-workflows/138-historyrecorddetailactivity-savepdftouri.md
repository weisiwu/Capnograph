# HistoryRecordDetailActivity.savePdfToUri

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L280）。
领域：页面函数。

## 实体定位

- 实体：`HistoryRecordDetailActivity.savePdfToUri`
- ID / 别名：copy PDF to Uri, 保存 PDF 到 Uri
- 源文件：`app/src/main/java/com/wldmedical/capnoeasy/pages/HistoryRecordDetailActivity.kt`
- 原始补充上下文：`.cursor/rules/project-memory.mdc`
- 备注：将 cache 中的 PDF 拷贝到用户选择的 Uri
...
copy PDF to Uri, 保存 PDF 到 Uri

## 关键字段 / 方法

- 主要实体或方法：`HistoryRecordDetailActivity.savePdfToUri`
