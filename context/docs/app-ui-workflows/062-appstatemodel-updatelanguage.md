# AppStateModel.updateLanguage

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L166）。
领域：状态函数。

## 实体定位

- 实体：`AppStateModel.updateLanguage`
- ID / 别名：locale state, 应用语言切换
...
- 备注：使用 AppCompatDelegate 切换 `zh`/`en` 并重建 Activity
...
- 主要实体或方法：`AppStateModel.updateLanguage`
