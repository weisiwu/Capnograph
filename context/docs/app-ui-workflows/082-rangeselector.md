...
定位入口：`context/entity-id-mapping.md`（L191）。
领域：组件。

## 实体定位

- 实体：RangeSelector
- ID / 别名：range slider, 报警阈值, 范围选择
...
通用滑块组件，RangeType.BOTH 使用 RangeSlider，RangeType.ONESIDE 使用 Slider，并在滑块附近绘制数值和单位。

## 关键 ID / 别名

range slider, 报警阈值, 范围选择
