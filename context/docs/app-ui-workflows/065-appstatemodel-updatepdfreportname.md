# AppStateModel.updatePdfReportName

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L169）。
领域：状态函数。

## 实体定位

- 实体：`AppStateModel.updatePdfReportName`
- ID / 别名：report name, PDF报告名
...
- 主要实体或方法：`AppStateModel.updatePdfReportName`
