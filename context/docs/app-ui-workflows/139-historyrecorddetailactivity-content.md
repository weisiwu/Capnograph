# HistoryRecordDetailActivity.Content

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L281）。
领域：UI 函数。

## 实体定位

- 实体：`HistoryRecordDetailActivity.Content`
- ID / 别名：record detail content, 详情内容
- 源文件：`app/src/main/java/com/wldmedical/capnoeasy/pages/HistoryRecordDetailActivity.kt`
- 原始补充上下文：`.cursor/rules/project-memory.mdc`
- 备注：读取 recordId、分页加载 chunk、渲染趋势图和波形图

## 补充职责

读取 recordId、分页加载 chunk、渲染趋势图和波形图。

## 关键 ID / 别名

record detail content, 详情内容

## 关键字段 / 方法

- 主要实体或方法：`HistoryRecordDetailActivity.Content`
