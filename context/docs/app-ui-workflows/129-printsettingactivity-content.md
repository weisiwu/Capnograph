...
- 备注：保存医院名、报告名、PDF 模板/水印、输出类型和详情趋势图开关

## 补充职责

保存医院名、报告名、PDF 正式/调试模板、水印开关/文字/透明度、输出类型和详情趋势图开关。
...
保存时同步更新 ViewModel 和 SharedPreferences；水印透明度输入按 Float 解析并裁剪到 `0..1`，解析失败时使用默认 `0.3`。
