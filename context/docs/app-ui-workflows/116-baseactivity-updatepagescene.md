# BaseActivity.updatePageScene

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L258）。
领域：页面函数。

## 实体定位

- 实体：`BaseActivity.updatePageScene`
- ID / 别名：page scene update, 页面场景同步
- 源文件：`app/src/main/java/com/wldmedical/capnoeasy/pages/BaseActivity.kt`
- 原始补充上下文：`.cursor/rules/project-memory.mdc`
- 备注：将 Activity 的 `pageScene` 写入 ViewModel
...
page scene update, 页面场景同步

## 关键字段 / 方法

- 主要实体或方法：`BaseActivity.updatePageScene`
