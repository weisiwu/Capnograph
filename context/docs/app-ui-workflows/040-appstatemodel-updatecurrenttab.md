# AppStateModel.updateCurrentTab

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L144）。
领域：状态函数。

## 实体定位

- 实体：`AppStateModel.updateCurrentTab`
- ID / 别名：tab state, 底栏 Tab 更新
...
- 主要实体或方法：`AppStateModel.updateCurrentTab`
