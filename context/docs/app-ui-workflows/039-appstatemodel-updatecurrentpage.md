# AppStateModel.updateCurrentPage

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L143）。
领域：状态函数。

## 实体定位

- 实体：`AppStateModel.updateCurrentPage`
- ID / 别名：page state, 页面状态更新
- 源文件：`app/src/main/java/com/wldmedical/capnoeasy/models/AppStateModel.kt`
- 原始补充上下文：`.cursor/rules/project-memory.mdc`
- 备注：更新 `NavBarComponentState.currentPage`

## 补充职责

更新 `NavBarComponentState.currentPage`。
...
- 主要实体或方法：`AppStateModel.updateCurrentPage`
