...
- 备注：PDF、PDF 模板/水印、异常波形上下文秒数、详情趋势图开关和热敏打印设置

## 补充职责

打印设置页，编辑医院名/报告名/PDF 水印文字和透明度、异常波形上下文秒数，用 TypeSwitch 控制正式/调试报告模板、水印开关、PDF/热敏输出和详情页趋势图展示，并写入 SharedPreferences。
...
PDF 和热敏打印设置；水印配置包含模板、开关、文字和透明度，透明度保存前限制到 `0..1`。PDF 异常上下文秒数仍在设置页保存并限制到 `10..300`，但当前 PDF 导出固定按 15 秒连续波形切段，不再使用该值。
