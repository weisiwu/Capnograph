# CapnoEasyApplication.getActivityCount

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L248）。
领域：应用入口。

## 实体定位

- 实体：`CapnoEasyApplication.getActivityCount`
- ID / 别名：activity count, 活动数量
- 源文件：`app/src/main/java/com/wldmedical/capnoeasy/CapnoEasyApplication.kt`
- 原始补充上下文：`.cursor/rules/project-memory.mdc`
- 备注：BaseActivity 返回拦截判断是否为最后页面

## 补充职责

BaseActivity 返回拦截判断是否为最后页面。
...
- 主要实体或方法：`CapnoEasyApplication.getActivityCount`
