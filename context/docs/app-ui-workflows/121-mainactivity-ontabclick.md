# MainActivity.onTabClick

来源批次：应用入口、页面、UI 与状态实体补充。
定位入口：`context/entity-id-mapping.md`（L263）。
领域：页面函数。

## 实体定位

- 实体：`MainActivity.onTabClick`
- ID / 别名：recording tab guard, 记录中切换拦截
...
recording tab guard, 记录中切换拦截

## 关键字段 / 方法

- 主要实体或方法：`MainActivity.onTabClick`
