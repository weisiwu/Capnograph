...
- 备注：实时图表、历史图表和打印 Bitmap 渲染；PDF 报告单连续 15 秒波形段由 Canvas 手绘

## 补充职责

折线图渲染库，覆盖实时、历史和打印图表；PDF 报告单连续 15 秒波形段由 `PDFKit` 用 Android Canvas 手绘，不再使用 MPAndroidChart 默认样式。
...
`EtCo2LineChart`、`HistoryRecordDetailActivity`、`HotmeltPinter` 使用 `LineChart` 和相关 data/axis API。`PDFKit` 仍接收 `LineChart` 作为任务入口参数，但报告单连续 15 秒波形段 bitmap 已改为 Canvas 手绘。
