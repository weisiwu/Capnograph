...
同名实体在定位表中出现两次：L48 是构建平台能力，L510 是构建/测试入口；两个实体文档分别记录不同语境。`./gradlew` 首次执行会下载 Gradle 8.10.2 到本机 `~/.gradle/wrapper/dists`，依赖安装/构建会继续写入 `~/.gradle` 缓存。若脚本缺少执行位，先在仓库根目录执行 `chmod +x gradlew`。
