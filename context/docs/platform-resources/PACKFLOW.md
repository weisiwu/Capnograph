...
Android debug APK 当前约 `105690613` bytes，飞书通知不要上传 APK 文件本体。`capno_packflow_agent` 默认发送 Packflow 下载链接 `/api/artifacts/<artifactId>/download`；跨机器或外网收件人需要通过 `PACKFLOW_PUBLIC_BASE_URL` 配置可访问的 Packflow 地址。
...
`capno_packflow_agent` 最小端到端验证可在 OMP 或测试 harness 中触发 `Android Debug APK`，并确认返回的 `details.build.status=success`、`details.artifacts[0].file_name=apps/android/app/build/outputs/apk/debug/app-debug.apk`、`details.artifacts[0].downloadUrl`、`details.notification.sent=true`。`capno_packflow_bot_command` 的无副作用验证用 `dryRun=true` 测试 `capno_packflow_agent`、`打包状态`、`最近 3 条打包`、`打包详情 <buildId>`、`打包配置`、`打 release 包` 等口令。
